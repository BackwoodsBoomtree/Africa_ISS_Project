...
df_all <- read.csv("G:/Africa/csv/ecoregions/mask_Dans/GOME2_Congo_Monthly_Mean/GOME2_Congo_Monthly_Means_2007-2018.csv", header = TRUE)
df_cs  <- read.csv("G:/Africa/csv/ecoregions/mask_Dans/GOME2_Congo_Monthly_Mean/GOME2_Congo_Monthly_Means_2007-2018_cs.csv", header = TRUE)

monthly_all_ts <- ts(df_all$SIF_740d, start = c(2007, 2), frequency = 12)
monthly_cs_ts  <- ts(df_cs$SIF_740d, start = c(2007, 2), frequency = 12)
...
# PLOTS
dates     <- seq(as.Date("2007/2/1"), as.Date("2018/1/1"), "months")
dates     <- format(dates, format = "%Y-%m")
x         <- 1:length(dates)
year_labs <- seq(2007, 2018)

cairo_pdf("G:/Africa/figs/trends_gome2_all.pdf", width = 7.5, height = 4)
