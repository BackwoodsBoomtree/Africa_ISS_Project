...
csv_list <- list.files("G:/Africa/csv/ecoregions/mask_Dans", pattern = "*.csv", full.names = TRUE)
x_labs   <- format(seq(as.Date("2019/1/1"), as.Date("2021/12/1"), "month"), "%Y-%m")
x        <- 1:36
err_col  <- "#8ab8e6"

cairo_pdf("G:/Africa/figs/timeseries_sifd_ecoregions_2.5km_mask.pdf", width = 20, height = 15)

par(mfrow = c(5, 3), oma=c(2.0,0.1,1.25,0.1))

for (i in 1:length(csv_list)) {
  
  f_name <- basename(csv_list[i])
  tit    <- substr(f_name, 1, nchar(f_name) - 19)
  tit    <- substr(tit, 21, nchar(tit))
