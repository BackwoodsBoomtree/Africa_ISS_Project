...
#### Output PDF name ####
out_name  <- "G:/Africa/figs/r_vs_precip_vpd_annual_difference_pearsons.pdf"
...
for (i in 1:length(trop_list)) {
  ...
  # Run regression
  spear_p <- cor.test(precip, t_sif, method = "pearson")
  spear_v <- cor.test(vpd, t_sif, method = "pearson")
...
for (i in 1:length(evi_list)) {
  ...
  # Run regression
  spear_p <- cor.test(precip, evi, method = "pearson")
  spear_v <- cor.test(vpd, evi, method = "pearson")
  reg_r_precip <- spear_p$estimate
  reg_p_precip <- round2(spear_p$p.value, 2, TRUE)
  reg_r_vpd    <- spear_v$estimate
  reg_p_vpd    <- round2(spear_v$p.value, 2, TRUE)
...
for (i in 1:length(nirv_list)) {
  ...
  # Run regression
  spear_p <- cor.test(precip, nirv, method = "pearson")
  spear_v <- cor.test(vpd, nirv, method = "pearson")
  reg_r_precip <- spear_p$estimate
  reg_p_precip <- round2(spear_p$p.value, 2, TRUE)
  reg_r_vpd    <- spear_v$estimate
  reg_p_vpd    <- round2(spear_v$p.value, 2, TRUE)
...
for (i in 1:length(ndvi_list)) {
  ...
  # Run regression
  spear_p <- cor.test(precip, ndvi, method = "pearson")
  spear_v <- cor.test(vpd, ndvi, method = "pearson")
  reg_r_precip <- spear_p$estimate
  reg_p_precip <- round2(spear_p$p.value, 2, TRUE)
  reg_r_vpd    <- spear_v$estimate
  reg_p_vpd    <- round2(spear_v$p.value, 2, TRUE)
...
for (i in 1:length(lswi_list)) {
  ...
  # Run regression
  spear_p <- cor.test(precip, lswi, method = "pearson")
  spear_v <- cor.test(vpd, lswi, method = "pearson")
  reg_r_precip <- spear_p$estimate
  reg_p_precip <- round2(spear_p$p.value, 2, TRUE)
  reg_r_vpd    <- spear_v$estimate
  reg_p_vpd    <- round2(spear_v$p.value, 2, TRUE)
...
# Labels
lab_r      <- bquote("Difference in Pearson's Corrleation Coefficient (VPD - Precip)")
...
par(mfrow = c(2, 5), oma=c(3.5,2,2,0.5))
...
for (i in 1:5) {
  ...
  # Run regression
  spear <- cor.test(precip_ann, df_r_diff[,i], method = "pearson")
...
for (i in 1:5) {
  ...
  # Run regression
  spear <- cor.test(precip_sd, df_r_diff[,i], method = "pearson")
  reg   <- lm(y ~ x, data = df_p)
  reg_r <- bquote(r[s]~" = "~.(round2(spear$estimate, 2, FALSE)))
  reg_p <- round2(spear$p.value, 2, TRUE)
