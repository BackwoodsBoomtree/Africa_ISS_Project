...
#### Output PDF name ####
out_name  <- "G:/Africa/figs/r_vs_precip_vpd_annual_v2.pdf"
...
for (i in 1:5) {
  ...
  # Read Data
  df_p     <- data.frame(precip_ann, df_r_precip[,i])
  colnames(df_p) <- c("x", "y")
  
  # Run regression
  spear <- cor.test(precip_ann, df_r_precip[,i], method = "spearman")
  ...
  plot(precip_ann, df_r_precip[,i], axes = FALSE, xaxt="n", yaxt="n", xlab = NA, ylab = NA, ylim = c(0, 1), pch = NA)
  polygon(c(rev(newx), newx), c(rev(preds[ ,3]), preds[ ,2]), col = 'grey80', border = NA)
  par(new=TRUE)
  plot(precip_ann, df_r_precip[,i], axes = FALSE, xaxt="n", yaxt="n", xlab = NA, ylab = NA, ylim = c(0, 1))
  abline(reg)
...
for (i in 1:5) {
  ...
  # Read Data
  df_p     <- data.frame(precip_ann, df_r_vpd[,i])
  colnames(df_p) <- c("x", "y")
  
  # Run regression
  spear <- cor.test(precip_ann, df_r_vpd[,i], method = "spearman")
  reg   <- lm(y ~ x, data = df_p)
  reg_r <- bquote(r[s]~" = "~.(round2(spear$estimate, 2, FALSE)))
  reg_p <- round2(spear$p.value, 2, TRUE)
  ...
  plot(precip_ann, df_r_vpd[,i], axes = FALSE, xaxt="n", yaxt="n", xlab = NA, ylab = NA, ylim = c(-1, 0), pch = NA)
  polygon(c(rev(newx), newx), c(rev(preds[ ,3]), preds[ ,2]), col = 'grey80', border = NA)
  par(new=TRUE)
  plot(precip_ann, df_r_vpd[,i], axes = FALSE, xaxt="n", yaxt="n", xlab = NA, ylab = NA, ylim = c(-1, 0))
  abline(reg)
  axis(1, tck = 0.03, mgp=c(3, 0.2, 0))
  axis(2, tck = 0.03, mgp=c(3, 0.2, 0), las = 2)
