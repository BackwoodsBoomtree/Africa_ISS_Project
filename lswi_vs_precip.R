...
#### Output PDF name ####
out_name  <- "G:/Africa/figs/lswi_vs_precip.pdf"
...
for (i in 1:length(lswi_list)) {
  ...
  plot(precip, lswi, axes = FALSE, xaxt="n", yaxt="n", xlab = NA, ylab = NA, xlim = c(0, max(precip)), ylim = c(0.2, 0.4), pch = NA)
  polygon(c(rev(newx), newx), c(rev(preds[ ,3]), preds[ ,2]), col = 'grey80', border = NA)
  par(new=TRUE)
  plot(precip, lswi, axes = FALSE, xaxt="n", yaxt="n", xlab = NA, ylab = NA, xlim = c(0, max(precip)), ylim = c(0.2, 0.4))
  abline(reg)
...
mtext(lab_lswi, side = 2, outer = TRUE, line = 1)
