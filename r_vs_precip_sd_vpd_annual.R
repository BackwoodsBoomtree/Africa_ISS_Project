...
#### Output PDF name ####
out_name  <- "G:/Africa/figs/r_vs_precip_sd_vpd_annual_v2.pdf"
...
for (i in 1:length(trop_list)) {
  ...
  vpd    <- read.csv(era5_csv_list[i])$vpd_mean[13:48]
...
for (i in 1:length(nirv_list)) {
  ...
  precip <- df_precip$mean[p_start[i]:p_end[i]]
  vpd    <- read.csv(era5_csv_list[i])$vpd_mean[13:48]
  df_p   <- data.frame(precip, nirv)
  df_v   <- data.frame(vpd, nirv)
  colnames(df_p) <- c("x", "y")
  colnames(df_v) <- c("x", "y")
  
  # Run regression
  spear_p <- cor.test(precip, nirv, method = "spearman")
  spear_v <- cor.test(vpd, nirv, method = "spearman")
  reg_r_precip <- spear_p$estimate
  reg_p_precip <- round2(spear_p$p.value, 2, TRUE)
  reg_r_vpd    <- spear_v$estimate
  reg_p_vpd    <- round2(spear_v$p.value, 2, TRUE)
  ...
  if (i == 1) {
    r_nirv_precip <- reg_r_precip
    p_nirv_precip <- reg_p_precip
    r_nirv_vpd    <- reg_r_vpd
    p_nirv_vpd    <- reg_p_vpd
  } else {
    r_nirv_precip <- c(r_nirv_precip, reg_r_precip)
    p_nirv_precip <- c(p_nirv_precip, reg_p_precip)
    r_nirv_vpd    <- c(r_nirv_vpd, reg_r_vpd)
    p_nirv_vpd    <- c(p_nirv_vpd, reg_p_vpd)
  }
}
...
for (i in 1:length(lswi_list)) {
  ...
  precip <- df_precip$mean[p_start[i]:p_end[i]]
  vpd    <- read.csv(era5_csv_list[i])$vpd_mean[13:48]
  df_p   <- data.frame(precip, lswi)
  df_v   <- data.frame(vpd, lswi)
  colnames(df_p) <- c("x", "y")
  colnames(df_v) <- c("x", "y")
  
  # Run regression
  spear_p <- cor.test(precip, lswi, method = "spearman")
  spear_v <- cor.test(vpd, lswi, method = "spearman")
  reg_r_precip <- spear_p$estimate
  reg_p_precip <- round2(spear_p$p.value, 2, TRUE)
  reg_r_vpd    <- spear_v$estimate
  reg_p_vpd    <- round2(spear_v$p.value, 2, TRUE)
  ...
  if (i == 1) {
    r_lswi_precip <- reg_r_precip
    p_lswi_precip <- reg_p_precip
    r_lswi_vpd    <- reg_r_vpd
    p_lswi_vpd    <- reg_p_vpd
  } else {
    r_lswi_precip <- c(r_lswi_precip, reg_r_precip)
    p_lswi_precip <- c(p_lswi_precip, reg_p_precip)
    r_lswi_vpd    <- c(r_lswi_vpd, reg_r_vpd)
    p_lswi_vpd    <- c(p_lswi_vpd, reg_p_vpd)
  }
}

# Put it all in a df
df_r_precip <- data.frame(r_tro_precip, r_evi_precip, r_nirv_precip, r_ndvi_precip, r_lswi_precip)
df_p_precip <- data.frame(p_tro_precip, p_evi_precip, p_nirv_precip, p_ndvi_precip, p_lswi_precip)
df_r_vpd <- data.frame(r_tro_vpd, r_evi_vpd, r_nirv_vpd, r_ndvi_vpd, r_lswi_vpd)
df_p_vpd <- data.frame(p_tro_vpd, p_evi_vpd, p_nirv_vpd, p_ndvi_vpd, p_lswi_vpd)

# Labels
lab_r      <- bquote("Spearman's Corrleation Coefficient")
lab_precip <- bquote("Standard Deviation of Monthly Precipitation (mm)")
titles     <- c("TROPOMI SIF", "EVI", "NIRv", "NDVI", "LSWI")

#### Plot ####
cairo_pdf(out_name, width = 12.5, height = 5.5)

par(mfrow = c(2, 5), oma=c(3.5,3,0,0.5))

# Plot precip
for (i in 1:5) {
  
  # Read Data
  df_p     <- data.frame(precip_sd, df_r_precip[,i])
  colnames(df_p) <- c("x", "y")
  
  # Run regression
  spear <- cor.test(precip_sd, df_r_precip[,i], method = "spearman")
  ...
  # Get title
  tit <- paste0(titles[i], " vs Precip")
  
  # Plot
  op <- par(mar = c(0,2,4,0.5))
  
  plot(precip_sd, df_r_precip[,i], axes = FALSE, xaxt="n", yaxt="n", xlab = NA, ylab = NA, ylim = c(0, 1), pch = NA)
  polygon(c(rev(newx), newx), c(rev(preds[ ,3]), preds[ ,2]), col = 'grey80', border = NA)
  par(new=TRUE)
  plot(precip_sd, df_r_precip[,i], axes = FALSE, xaxt="n", yaxt="n", xlab = NA, ylab = NA, ylim = c(0, 1))
  abline(reg)
...
for (i in 1:5) {
  ...
  # Read Data
  df_p     <- data.frame(precip_sd, df_r_vpd[,i])
  colnames(df_p) <- c("x", "y")
  
  # Run regression
  spear <- cor.test(precip_sd, df_r_vpd[,i], method = "spearman")
  reg   <- lm(y ~ x, data = df_p)
  reg_r <- bquote(r[s]~" = "~.(round2(spear$estimate, 2, FALSE)))
  reg_p <- round2(spear$p.value, 2, TRUE)
  ...
  # Get title
  tit <- paste0(titles[i], " vs VPD")
  
  # Plot
  op <- par(mar = c(0,2,4,0.5))
  
  plot(precip_sd, df_r_vpd[,i], axes = FALSE, xaxt="n", yaxt="n", xlab = NA, ylab = NA, ylim = c(-1, 0), pch = NA)
  polygon(c(rev(newx), newx), c(rev(preds[ ,3]), preds[ ,2]), col = 'grey80', border = NA)
  par(new=TRUE)
  plot(precip_sd, df_r_vpd[,i], axes = FALSE, xaxt="n", yaxt="n", xlab = NA, ylab = NA, ylim = c(-1, 0))
  abline(reg)
  axis(1, tck = 0.03, mgp=c(3, 0.2, 0))
  axis(2, tck = 0.03, mgp=c(3, 0.2, 0), las = 2)
