...
#### Output PDF name ####
out_name  <- "G:/Africa/figs/ndvi_vs_precip.pdf"
...
# Remove Niger forest
ndvi_list <- c(ndvi_list[1:6], ndvi_list[9:12])
...
for (i in 1:length(ndvi_list)) {
  ...
  # Read Data
  ndvi   <- read.csv(ndvi_list[1])$NDVI / 10000
  ndvi   <- ndvi[13:48]
  ...
  plot(precip, ndvi, axes = FALSE, xaxt="n", yaxt="n", xlab = NA, ylab = NA, xlim = c(0, max(precip)), ylim = c(0.6, 1.0), pch = NA)
  polygon(c(rev(newx), newx), c(rev(preds[ ,3]), preds[ ,2]), col = 'grey80', border = NA)
  par(new=TRUE)
  plot(precip, ndvi, axes = FALSE, xaxt="n", yaxt="n", xlab = NA, ylab = NA, xlim = c(0, max(precip)), ylim = c(0.6, 1.0))
  abline(reg)
...
mtext(lab_ndvi, side = 2, outer = TRUE, line = 1)
