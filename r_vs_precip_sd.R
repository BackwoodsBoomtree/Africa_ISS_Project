...
for (i in 1:5) {
  ...
  # Read Data
  df     <- data.frame(precip_sd, df_r[,i])
  colnames(df) <- c("x", "y")
  
  # Run regression
  spear <- cor.test(precip_sd, df_r[,i], method = "spearman")
  reg   <- lm(y ~ x, data = df)
  ...
  plot(precip_sd, df_r[,i], axes = FALSE, xaxt="n", yaxt="n", xlab = NA, ylab = NA, ylim = c(0, 1), pch = NA)
  polygon(c(rev(newx), newx), c(rev(preds[ ,3]), preds[ ,2]), col = 'grey80', border = NA)
  par(new=TRUE)
  plot(precip_sd, df_r[,i], axes = FALSE, xaxt="n", yaxt="n", xlab = NA, ylab = NA, ylim = c(0, 1))
  abline(reg)
