...
vi_list          <- c("EVI", "LSWI", "NDVI") # Alphabetical
...
for (i in 1:length(eco_list)) {
  ...
  par(new = TRUE)
  barplot(precip, lwd = 3, col = "#648FFF", axes = FALSE, xlab = "", ylab = "", ylim = c(0, 900), space = 0, border = NA)
...
legend("topright", legend = c("NDVI", "EVI", lab_sif, "LSWI", lab_precip),
       col = c("#dc267f", "#FE6100", "black", "#ffb000", "#648FFF"), lty = c(1, 1, 1, 1, NA), 
       pch = c(NA, NA, NA, NA, 15), lwd = 3, cex = 2)
